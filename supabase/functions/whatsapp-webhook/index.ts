...
// Initiative 1.5 — CONTEXTUAL_ASK / WEB_RESEARCH / SCHEDULE_CALENDAR
// (semantic search over saved data + Perplexity augmentation) and
// WEB_SEARCH (Perplexity-powered external search). Both construct
// `save_artifact` PendingOffers when responses suggest a save — the
// artifact-freezing pattern survives intervening CHAT turns.
import { makeContextualAskHandler } from "./handlers/contextual-ask.ts";
import { makeWebSearchHandler } from "./handlers/web-search.ts";
// Initiative 1.6 — CREATE (brain-dump) handler. Pronoun resolution,
// topical-followup attach, process-note + multi/single-note insert
// with encryption + list inheritance, post-insert duplicate detection,
// proactive bridge offer, localized confirmation.
import { makeCreateNoteHandler } from "./handlers/create-note.ts";
// Initiative 1.7 — EXPENSE (text + media/process-receipt paths, budget
// check) and PARTNER_MESSAGE (couple resolution, dedupe, task create,
// trust gate, Meta direct send with template fallback).
import { makeExpenseHandler } from "./handlers/expense.ts";
import { makePartnerMessageHandler } from "./handlers/partner-message.ts";
// Initiative 1.7b — TASK_ACTION handler. The last big intent block
// (~1,191 lines) lifted out of this monolith. Owns the task-resolution
// priority chain (quoted ctx → relative ref → ordinal → AI UUID →
// semantic search → session entity → outbound ctx → compound
...
// Phase 3.5 — pattern learning. `findMatchingPatterns` /
// `MatchedPattern` / conflict-detector / bulk-resolver / extractTimeOnly
// moved to handlers/task-action.ts (Initiative 1.7b). Only
// recordReschedulePattern is still wired here (called from confirmation
// handling for set_due_date confirmations).
import { recordReschedulePattern } from "../_shared/pattern-detector.ts";
...
Description: "${parsedExpense.description}"`;
    ...
    // ========================================================================
    // TASK_ACTION — handler in ./handlers/task-action.ts (Initiative 1.7b).
    // ========================================================================
    if (intent === 'TASK_ACTION') {
      const r = await makeTaskActionHandler({
        t, generateEmbedding, saveReferencedEntity,
      })({
        supabase, userId, userLang, userTimezone: profile.timezone || 'America/New_York',
        profile: profile as any, coupleId, effectiveCoupleId, session: session as any,
      ...
      r.after_reply?.forEach((cb) => cb().catch((e) => console.warn('[TASK_ACTION] after-reply:', e)));
      return reply(r.text);
    }

    // ========================================================================
    // EXPENSE — handler in ./handlers/expense.ts (Initiative 1.7).
    // ========================================================================
    if (intent === 'EXPENSE') {
      const r = await makeExpenseHandler({ callAI, t })({
        supabase, userId, userLang, userTimezone: profile.timezone || 'America/New_York',
        profile: profile as any, coupleId, effectiveCoupleId, session: session as any,
        messageBody, cleanMessage, effectiveMessage, mediaUrls, mediaTypes,
        wamid, inboundNoteSource, quotedMessageId: quotedMessageId ?? null,
    ...
    // ========================================================================
    // CONTEXTUAL_ASK / WEB_RESEARCH / SCHEDULE_CALENDAR
    //   handler in ./handlers/contextual-ask.ts (Initiative 1.5).
    // ========================================================================
    if (intent === 'CONTEXTUAL_ASK' || intent === 'WEB_RESEARCH' || intent === 'SCHEDULE_CALENDAR') {
      const r = await makeContextualAskHandler({
        callAI, t, generateEmbedding, saveReferencedEntity,
      })({
        supabase, userId, userLang, userTimezone: profile.timezone || 'America/New_York',
        profile: profile as any, coupleId, effectiveCoupleId, session: session as any,
        messageBody, cleanMessage, effectiveMessage, mediaUrls, mediaTypes,
      ...
      r.after_reply?.forEach((cb) => cb().catch((e) => console.warn('[CONTEXTUAL_ASK] after-reply:', e)));
      return reply(r.text.slice(0, r.max_length ?? 1500));
    }

    // ========================================================================
    // WEB_SEARCH — handler in ./handlers/web-search.ts (Initiative 1.5).
    // ========================================================================
    if (intent === 'WEB_SEARCH') {
      const r = await makeWebSearchHandler({
        callAI, t, saveReferencedEntity,
      })({
        supabase, userId, userLang, userTimezone: profile.timezone || 'America/New_York',
        profile: profile as any, coupleId, effectiveCoupleId, session: session as any,
        messageBody, cleanMessage, effectiveMessage, mediaUrls, mediaTypes,
      ...
      r.after_reply?.forEach((cb) => cb().catch((e) => console.warn('[WEB_SEARCH] after-reply:', e)));
      return reply(r.text.slice(0, r.max_length ?? 1500));
    }
    ...
    // ========================================================================
    // PARTNER_MESSAGE — handler in ./handlers/partner-message.ts (Initiative 1.7).
    // ========================================================================
    if (intent === 'PARTNER_MESSAGE') {
      const r = await makePartnerMessageHandler({
        t, generateEmbedding, saveReferencedEntity,
      })({
        supabase, userId, userLang, userTimezone: profile.timezone || 'America/New_York',
        profile: profile as any, coupleId, effectiveCoupleId, session: session as any,
      ...
      r.after_reply?.forEach((cb) => cb().catch((e) => console.warn('[PARTNER_MESSAGE] after-reply:', e)));
      return reply(r.text);
    }
...
- End with an actionable suggestion`;
    ...
    // ========================================================================
    // CREATE — default fall-through handler (Initiative 1.6).
    // Catches any intent that didn't return earlier. Owns the brain-dump
    // path: pronoun resolution, topical-followup attach, process-note
    // invoke, multi/single-note insert with encryption + list inheritance,
    // post-insert duplicate detection, proactive bridge offer, and the
    // localized confirmation. Lives in ./handlers/create-note.ts.
    // ========================================================================
    {
      const r = await makeCreateNoteHandler({
        t, generateEmbedding, saveReferencedEntity,
      ...
      r.after_reply?.forEach((cb) => cb().catch((e) => console.warn('[CREATE] after-reply:', e)));
      return reply(r.text);
    }
