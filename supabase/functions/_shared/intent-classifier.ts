...
7. **Questions about data = contextual_ask.** "When is X?", "What did I save about Y?", "Do I have any Z?" → contextual_ask. But ONLY if the user is clearly asking a QUESTION about existing data. A statement with a date/time/location is a CREATE, NOT contextual_ask.
8. **New items with details = create.** If the message contains a date, time, location, or appointment-like details AND does NOT match any existing task in ACTIVE TASKS, it is always "create". Example: "Oliva vet visit at Banfield on 20-Mar at 5pm" → CREATE. The user is telling Olive to SAVE this, not asking if it exists.
9. **Ambiguity → lean towards the most helpful intent.** If someone says "groceries" with no verb, check context: after "show me" → search. After nothing → probably search (they want to see their grocery list). Only classify as "create" if it clearly reads as a new item to save.
10. **Language:** The user speaks ${userLanguage}. Understand their message natively in that language.
