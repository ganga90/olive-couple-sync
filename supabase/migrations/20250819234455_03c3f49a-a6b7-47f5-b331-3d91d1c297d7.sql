-- Step 2: Update Database Schema for New Clerk-Supabase Integration (Fixed Order)

-- 1. First drop all policies that depend on get_clerk_user_id()
...
-- 2. Now drop the custom functions
DROP FUNCTION IF EXISTS public.get_clerk_user_id();
DROP FUNCTION IF EXISTS public.debug_jwt_claims();

-- 3. Update table structures: Change text columns to uuid for Clerk user IDs
...
-- 4. Recreate all policies using auth.uid()

-- Recreate clerk_profiles policies
...
-- Recreate clerk_couple_members policies
...
-- Recreate clerk_couples policies
...
-- 5. Update the trigger function to use the updated column types
