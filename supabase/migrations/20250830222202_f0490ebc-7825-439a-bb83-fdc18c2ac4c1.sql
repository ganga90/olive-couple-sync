...
-- 2) Drop all existing policies that reference the role column
...
DROP POLICY IF EXISTS memberships_select_mine ON public.clerk_couple_members;
DROP POLICY IF EXISTS memberships_manage ON public.clerk_couple_members;

-- 3) Ensure clerk_couple_members.role uses the enum
DO $$
DECLARE
    col_type text;
BEGIN
...
-- 6) Recreate RLS policies with proper enum comparisons
...
CREATE POLICY memberships_select_mine
ON public.clerk_couple_members
FOR SELECT TO authenticated
USING (
  (user_id = public.jwt_sub()) OR 
  EXISTS (
