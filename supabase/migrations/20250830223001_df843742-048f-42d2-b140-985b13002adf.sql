...
-- 2) Drop ALL existing policies on clerk_couple_members table
DO $$
DECLARE
    policy_rec RECORD;
BEGIN
    ...
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON public.clerk_couple_members', policy_rec.policyname);
    END LOOP;
END $$;

-- 3) Now safely alter the column type
DO $$
DECLARE
    current_type text;
BEGIN
...
-- Make sure PostgREST reloads the schema
