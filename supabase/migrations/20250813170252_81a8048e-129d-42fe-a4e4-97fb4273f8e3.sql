...
create type if not exists public.invite_status as enum ('pending', 'accepted', 'revoked');
create type if not exists public.note_priority as enum ('low','medium','high');
...
create or replace trigger set_couples_updated_at
...
create or replace trigger add_creator_as_member
...
create or replace trigger validate_invite_expiry
...
create policy if not exists "Members can view their couples" on public.couples
for select using (exists (
  select 1 from public.couple_members m
  where m.couple_id = couples.id and m.user_id = auth.uid()
));
create policy if not exists "Users can create couples" on public.couples
for insert with check (created_by = auth.uid());
create policy if not exists "Members can update their couples" on public.couples
for update using (exists (
  select 1 from public.couple_members m
  where m.couple_id = couples.id and m.user_id = auth.uid()
));
create policy if not exists "Owners can delete their couples" on public.couples
...
create policy if not exists "Users can view their memberships" on public.couple_members
for select using (user_id = auth.uid());
create policy if not exists "Owners can add members" on public.couple_members
for insert with check (exists (
  select 1 from public.couple_members m
  where m.couple_id = couple_members.couple_id and m.user_id = auth.uid() and m.role = 'owner'
));
create policy if not exists "Owners can update members" on public.couple_members
for update using (exists (
  select 1 from public.couple_members m
  where m.couple_id = couple_members.couple_id and m.user_id = auth.uid() and m.role = 'owner'
));
create policy if not exists "Owners can remove members" on public.couple_members
for delete using (exists (
  select 1 from public.couple_members m
  where m.couple_id = couple_members.couple_id and m.user_id = auth.uid() and m.role = 'owner'
));
...
create policy if not exists "Owners can manage invites" on public.invites
...
create policy if not exists "Inviter can view invite" on public.invites
for select using (invited_by = auth.uid());

-- notes: members can CRUD within their couple
create policy if not exists "Members can view notes in their couples" on public.notes
for select using (exists (
  select 1 from public.couple_members m
  where m.couple_id = notes.couple_id and m.user_id = auth.uid()
));
create policy if not exists "Members can insert notes in their couples" on public.notes
for insert with check (exists (
  select 1 from public.couple_members m
  where m.couple_id = notes.couple_id and m.user_id = auth.uid()
) and (author_id = auth.uid()));
create policy if not exists "Members can update notes in their couples" on public.notes
for update using (exists (
  select 1 from public.couple_members m
  where m.couple_id = notes.couple_id and m.user_id = auth.uid()
));
create policy if not exists "Members can delete notes in their couples" on public.notes
for delete using (exists (
  select 1 from public.couple_members m
  where m.couple_id = notes.couple_id and m.user_id = auth.uid()
));
