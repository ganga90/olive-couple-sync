-- 1) Create member_role enum if it doesn't exist
...
-- 3) Create deny-all policy for clerk_invites (we'll use RPCs only)
CREATE POLICY "deny_all_clerk_invites" ON public.clerk_invites 
FOR ALL USING (false) WITH CHECK (false);

-- 4) Helper function to check couple membership safely
...
-- 5) Function to validate invite token (read-only)
