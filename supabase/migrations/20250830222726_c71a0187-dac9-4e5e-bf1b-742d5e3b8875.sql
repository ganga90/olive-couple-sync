-- Fix member_role enum issues by dropping all policies first
...
-- 2) Drop ALL policies on clerk_couple_members table to allow column type change
DO $$
DECLARE
    policy_rec RECORD;
BEGIN
...
-- 3) Now safely convert the column type
DO $$
DECLARE
    current_type text;
BEGIN
...
-- 6) Recreate create_couple function with proper enum usage
...
-- 7) Recreate essential policies with proper enum comparisons
